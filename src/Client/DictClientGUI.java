...
public class DictClientGUI {
	...
	private Boolean isValid(String word, String meaning, int command) {
		if (word.equals("")) {
			JOptionPane.showMessageDialog(frame, "Word is required.", "Warning", JOptionPane.WARNING_MESSAGE);
			return false;
		} else if (command == StateCode.ADD && meaning.equals("")) {
			JOptionPane.showMessageDialog(frame, "Word meaning is required.", "Warning", JOptionPane.WARNING_MESSAGE);
			return false;
		}
		return true;
	}
	...
	private void initialize() {
		...
		frame.setBounds(635, 340, 450, 300);
		...
		//lblWord.setFont(new Font("Serif", Font.PLAIN, 14));
		//lblWord.setPreferredSize(new Dimension(250, 100));
